...
from common.cleaning import defensive_clean_series  # noqa: E402
...
def _clean_excel(df_m_raw, df_l_raw):
    ...
    prov_raw = pd.to_numeric(
        df_l_raw.get("提撥率", pd.Series(0, index=df_l_raw.index, dtype=float)), errors="coerce"
    )
...
def load_data_from_bytes(excel_bytes, csv_bytes=None):
        ...
        with pd.ExcelFile(io.BytesIO(excel_bytes)) as xls:
            ...
            for key, name in SHEETS.items():
                if name not in sheet_names:
                    raise ValueError(f"Excel 缺少工作表「{name}」（應為 {list(SHEETS.values())}）")
            ...
            pd.read_excel(
                xls, sheet_name=SHEETS["REGION"], dtype={"社名": str, "區域": str, "密碼": str}
            )  # validate sheet exists; result unused in report tool
    except ValueError:
        raise
    except Exception as e:
        raise ValueError(f"無法讀取 Excel 檔案，請確認上傳的是正確的「資料庫.xlsx」（{e}）") from e

    df_l_raw = df_l_raw.rename(columns={"收支比": "開支比"})
    required_m = {"社號", "社名", "年月", "社員數", "股金", "貸放比", "儲蓄率"}
    missing_m = required_m - set(df_m_raw.columns)
    if missing_m:
        raise ValueError(f"社務資料工作表缺少欄位：{', '.join(sorted(missing_m))}")
    required_l = {"社號", "社名", "年月", "開支比", "逾放比", "逾期貸款"}
    missing_l = required_l - set(df_l_raw.columns)
    if missing_l:
        raise ValueError(f"放款資料工作表缺少欄位：{', '.join(sorted(missing_l))}")

    df_m, df_l = _clean_excel(df_m_raw, df_l_raw)

    if df_m.empty:
        raise ValueError(
            "清洗後社務資料無有效資料，請檢查「年月」格式是否正確（應為 5 碼民國年月，如 11504）"
        )
    ...
    if csv_bytes:
        try:
            df_csv = pd.read_csv(
                io.BytesIO(csv_bytes), encoding="utf-8-sig", dtype={"社號": str, "年月": str}
            )
            ...
            if not required_csv.issubset(set(df_csv.columns)):
                ...
                df_csv["會計科目"] = (
                    df_csv["會計科目"].astype(str).str.replace(".0", "", regex=False)
                )
        except Exception as e:
            print(f"  CSV 讀取失敗：{e}")
...
def extract_union_data(df_m, df_l, df_csv, union_id):
    ...
    eProv = (
        float(union_l["提撥率"].iloc[-1])
        if not union_l.empty and "提撥率" in union_l.columns
        else 0.0
    )
    ...
    if c3:
        notes.append("高逾放且惡化")
    if c4:
        notes.append("人數連三年衰退")
    if c5:
        notes.append("股金連三年衰退")
...
def compute_ovd_stats(d):
    ...
    return dict(
        ...
        avg12=avg12,
        hist_max=hist_max,
        hist_max_d=hist_max_d,
        hist_min=hist_min,
        hist_min_d=hist_min_d,
        months_warn=months_warn,
